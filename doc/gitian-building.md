...
*Setup instructions for a Gitian build of Constella Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Constella
...
- [Building Constella Core](#building-constella-core)
...
Clone the git repositories for constella and Gitian.
...
in the constella repository under 'Fetch and create inputs' to install sources which require
...
To build Constella Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the constella repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/constella/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/constella-project/constella
...
For example:
```bash
URL=https://github.com/thrasher-/constella.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit constella=${COMMIT} --url constella=${URL} ../constella/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit constella=${COMMIT} --url constella=${URL} ../constella/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit constella=${COMMIT} --url constella=${URL} ../constella/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the constella git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../constella/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/constella
SIGPATH=/some/root/path/constella-detached-sigs

./bin/gbuild --url constella=${BTCPATH},signature=${SIGPATH} ../constella/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/constella-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/constella-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/constella-osx-build.assert
```
...
[constella-project/gitian.sigs.ltc](https://github.com/constella-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
